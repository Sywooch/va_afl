...
class Rating
{
    ...
    public function __construct($records, $mouth, $year)
    {
        ...
        foreach ($records->all() as $record) {
            ...
            $preRating = $pos + (int)Suspensions::overPeriod($record->user_id, $mouth, $year)->count();
            if($preRating <= 0){
                $preRating = 999;
            }
